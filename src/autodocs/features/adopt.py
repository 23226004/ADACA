...
코드를 옮기는 판단(이 파일이 L1 인가 L2 인가)은 AI/사람이 한다. 엔진은
  ...
  2) 소스 파일 목록과 계층 미분류 현황을 계획서(ADOPT_PLAN.md)로 내놓는다.
...
def run(manifest: dict, root: Path, profile: Profile) -> tuple[init.InitResult, str]:
    snap_before = fs.scan(root)
    res = init.run(manifest, root, profile)
    resolver = _Resolver(layout.layer_dirs(manifest, profile))
    unclassified = sorted(f for f in snap_before.source_files if resolver.layer_of_path(f) is None)
    plan = _plan(manifest, profile, unclassified)
    if fs.write_text(root / PLAN_REL, plan):
